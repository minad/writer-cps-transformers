...
-- The 'return' function produces the output 'mempty', while @>>=@
-- combines the outputs of the subcomputations using 'mappend'.
newtype WriterT w m a = WriterT { unWriterT :: m (StrictPair a w) }

writerT :: Functor m => m (a, w) -> WriterT w m a
writerT = WriterT . fmap strict

runWriterT :: Functor m => WriterT w m a -> m (a, w)
runWriterT = fmap unstrict . unWriterT

data StrictPair a w = StrictPair !a !w
  deriving (Eq, Ord)

strict :: (a, w) -> StrictPair a w
strict (a, w) = StrictPair a w

unstrict :: StrictPair a w -> (a, w)
unstrict (StrictPair a w) = (a, w)
...
mapWriterT :: (Functor m, Functor n) => (m (a, w) -> n (b, w')) -> WriterT w m a -> WriterT w' n b
...
instance (Foldable f, Functor f) => Foldable (WriterT w f) where
...
instance (Monoid w, MonadZip m) => MonadZip (WriterT w m) where
    mzipWith f x y = writerT $
        mzipWith (\ (a, w) (b, w') -> (f a b, w `mappend` w')) (runWriterT x) (runWriterT y)
    {-# INLINE mzipWith #-}
#endif
...
liftCallCC :: (Monoid w, Functor m) => CallCC m (a,w) (b,w) -> CallCC (WriterT w m) a b
...
liftCatch :: Functor m => Catch e m (a,w) -> Catch e (WriterT w m) a
liftCatch catchE m h =
    writerT $ runWriterT m `catchE` \ e -> runWriterT (h e)
{-# INLINE liftCatch #-}

deriveEq2 ''StrictPair
deriveOrd2 ''StrictPair
deriveShow2 ''StrictPair
deriveRead2 ''StrictPair
